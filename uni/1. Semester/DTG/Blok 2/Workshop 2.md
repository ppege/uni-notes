...
$$
Since the codomain $\mathbb{Z}_{\geq -1}$ is an infinite set, and the codomain $A\bigtimes\mathbb{Z}$ is finite, then $f$ is definitionally not surjective.
### Bijective property
...
If we see the witnesses as $C=3, k=2$, then our big O case is $|f(x)|\leq C|g(x)|\Leftrightarrow |2n+2|\leq 3\cdot n$ where $x>2$. Our big Omega case, with witnesses $C=2,k=0$ is $|f(x)| \geq C|g(x)|\Leftrightarrow |2n+2| \geq 2\cdot n$ where $x>0$. With this we have shown that the worst case for linear search is in big Theta of $n$.
